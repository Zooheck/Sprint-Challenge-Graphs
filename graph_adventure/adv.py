...
def bfs(starting_room):
    ...
    while q.size() > 0:
        ...
        # exits = 16's exits {'n': something, 's': something}
        exits = visited[(room[-1])]
        num_of_exits = len(exits)
        unexplored_path = available_directions(
            visited[(room[-1])], num_of_exits)
        if unexplored_path == False and room[-1] not in checked:
            checked.add(room[-1])
            for neighbor in visited[(room[-1])]:
                q.enqueue([*room, visited[(room[-1])][neighbor]])
        elif unexplored_path != False and room[-1] not in checked:
            checked.add(room[-1])
            next_room = [*room, visited[(room[-1])][unexplored_path]]
            break
    for i in range(1, len(next_room) - 1):
        for direction in visited[next_room[i - 1]]:
            if visited[next_room[i - 1]][direction] == next_room[i]:
