...
def available_directions(room, num_of_exits):
...
def bfs(starting_room):
    ...
    while q.size() > 0:
        room = q.dequeue() # room = [17, 16]
        exits = visited[(room[-1])] # exits = 16's exits {'n': something, 's': something}
        num_of_exits = len(exits)
        unexplored_path = available_directions(visited[(room[-1])], num_of_exits)
        if unexplored_path == False and room[-1] not in checked:
            checked.add(room[-1])
            for neighbor in visited[(room[-1]):
                q.enqueue([*room, visited[(room[-1])][neighbor]]) # [17, 16]
                # path[(visited[(room[-1])[neighbor])] = [neighbor]
        elif unexplored_path != False and room[-1] not in checked:
            checked.add(room[-1])
            next_room = [*room, visited[(room[-1])][unexplored_path]]
            break
    # next room is a list of numbers (rooms)
    # loop through the list, starting with second item
        # look at the previous item's entry in visited dictionary
        # if the previous item has a direction which takes us to current item, append it to the path list
...
while len(visited) < len(world.rooms):
    ...
    if current_room not in visited:
        ...
        if available_directions(visited[current_room], len(exits)) is not False:
            ...
            player.travel(travel_direction)
            visited[previous_room][travel_direction] = player.currentRoom.id
        # if we are in a previously visited room and both exits have been taken...
        else:
            current_room = player.currentRoom.id
            bfs(current_room)
            ...
            #                     q.enqueue(new_room)
                ...
                # check if it's a question marke
            # if we are hitting a wall...
            # backtracking = True
            # while backtracking == True:
            #     # pop off last element of move history
            #     backtrack_direction = opposite_direction(move_history.pop())
            #     # travel in that direction
            #     traversalPath.append(travel_direction)
            #     player.travel(backtrack_direction)
            #     if available_directions(visited[current_room], len(exits)) != False:
            #         backtracking = False
                # backtracking = False
                # if new room has unexplored exit, take it,
